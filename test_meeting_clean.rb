# Test du système de rencontre avec nettoyage

puts '🧹 Nettoyage des données de test...'
User.where(email: ['test1@example.com', 'test2@example.com']).each do |user|
  user.user_positions.destroy_all
  user.walking_meetings_as_a.destroy_all
  user.walking_meetings_as_b.destroy_all
  user.circuits.each { |c| c.walking.destroy }
  user.circuits.destroy_all
end
